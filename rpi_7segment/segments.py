...
class Segments:
    def __init__(self, bcm_gpio_clock=11, bcm_gpio_latch=13, bcm_gpio_data=14, num_displays=7, debug=False, offline=False):
        ...
        if not self._is_offline:
            ...
            GPIO.setup(self._segment_clock, GPIO.OUT)
            GPIO.setup(self._segment_data, GPIO.OUT)
            GPIO.setup(self._segment_latch, GPIO.OUT)

            GPIO.output(self._segment_clock, GPIO.LOW)
            GPIO.output(self._segment_data, GPIO.LOW)
            GPIO.output(self._segment_latch, GPIO.LOW)
    ...
    def post_character(self, symbol, show_decimal=False):
        ...
        elif symbol == "O": segments = a | b | c | d | e | f
        ...
        while y < self._num_segments:
            if not self._is_offline:
                GPIO.output(self._segment_clock, GPIO.LOW)
                GPIO.output(self._segment_data, segments & 1 << (7 - y))
                GPIO.output(self._segment_clock, GPIO.HIGH)
            y += 1
...
def run_demo_pattern(segments):
    while True:
        segments.show("Hello!")
        time.sleep(2)

        segments.show(1234567)
        time.sleep(2)

        segments.show(-123.456)
        time.sleep(2)

        segments.clear()
        time.sleep(1)

        segments.show("Hello world!")
...
if __name__ == '__main__':
    ...
    # Setup segments:
    # Offline == Run on actual devices / enable GPIO
    # Debug == Print debug state
    _segments = Segments(debug=True, offline=False)

    # Signal handler, for e.g. clean shutdown when ctrl+c is hit in the demo pattern:
